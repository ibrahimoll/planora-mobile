...
class _ProjectDetailScreenState extends State<ProjectDetailScreen> {
  ...
  Future<void> _refresh() async {
    ...
    try {
      ...
      try { plans = await _insightsApi.getAiPlanHistory(project); } catch (_) {}
  ...
  int _compareTasks(TaskListItem left, TaskListItem right) {
    ...
    if (leftTask.isCompleted != rightTask.isCompleted) return leftTask.isCompleted ? 1 : -1;
    if (leftTask.isOverdue != rightTask.isOverdue) return leftTask.isOverdue ? -1 : 1;
  ...
  Widget _brief(String value) {
    ...
    return Container(width: double.infinity, padding: const EdgeInsets.all(11), decoration: BoxDecoration(color: colors.primary.withOpacity(.05), borderRadius: BorderRadius.circular(16), border: Border.all(color: colors.primary.withOpacity(.10))), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(children: [Icon(Icons.auto_awesome_rounded, size: 16, color: colors.primary), const SizedBox(width: 6), Text('AI planning brief', style: _bold())]), const SizedBox(height: 7), Text(cleaned, style: _muted())]));
  }

  Widget _statsGrid() => GridView.count(shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), crossAxisCount: 2, childAspectRatio: 2.45, mainAxisSpacing: 8, crossAxisSpacing: 8, children: [_stat('Tasks', '${_tasks.length}', Icons.list_alt_rounded), _stat('Completed', '$_doneTasks', Icons.check_circle_rounded), _stat('Overdue', '$_overdueTasks', Icons.timer_off_rounded), _stat('Members', '${_members.length}', Icons.groups_rounded)]);
  ...
  Widget _tasksCard() {
    ...
    return _section('Project Tasks', '${filtered.length} showing • $_activeTasks active • $_overdueTasks overdue', Icons.task_alt_rounded, Column(crossAxisAlignment: CrossAxisAlignment.start, children: [_taskSummaryStrip(), const SizedBox(height: 12), _taskFilters(), const SizedBox(height: 14), if (_tasks.isEmpty) _emptyState('No tasks in this project yet.', 'Generate a plan or add tasks from the main Tasks screen.') else if (filtered.isEmpty) _emptyState('No tasks match this filter.', 'Try another filter above.') else ...[if (todo.isNotEmpty) _taskGroup('To Do', todo, Icons.radio_button_unchecked_rounded), if (doing.isNotEmpty) _taskGroup('In Progress', doing, Icons.timelapse_rounded), if (blocked.isNotEmpty) _taskGroup('Blocked', blocked, Icons.block_rounded), if (done.isNotEmpty) _taskGroup('Completed', done, Icons.check_circle_rounded)]]));
  }

  Widget _taskSummaryStrip() => Row(children: [Expanded(child: _miniStat('All', '${_tasks.length}', Icons.list_alt_rounded)), const SizedBox(width: 8), Expanded(child: _miniStat('Active', '$_activeTasks', Icons.bolt_rounded)), const SizedBox(width: 8), Expanded(child: _miniStat('Done', '$_doneTasks', Icons.check_rounded))]);

  Widget _taskFilters() {
    const labels = ['All', 'Active', 'Overdue', 'Done'];
    final colors = Theme.of(context).colorScheme;
    return SingleChildScrollView(scrollDirection: Axis.horizontal, child: Row(children: [for (var i = 0; i < labels.length; i++) ...[ChoiceChip(selected: _taskFilterIndex == i, label: Text(labels[i]), labelStyle: TextStyle(fontWeight: FontWeight.w900, color: _taskFilterIndex == i ? colors.onPrimary : colors.onSurfaceVariant), selectedColor: colors.primary, backgroundColor: colors.surfaceVariant.withOpacity(.45), side: BorderSide(color: _taskFilterIndex == i ? Colors.transparent : colors.outlineVariant.withOpacity(.55)), onSelected: (_) => setState(() => _taskFilterIndex = i)), if (i != labels.length - 1) const SizedBox(width: 8)]]));
  }

  Widget _taskGroup(String title, List<TaskListItem> items, IconData icon) => Padding(padding: const EdgeInsets.only(bottom: 12), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(children: [Icon(icon, color: Theme.of(context).colorScheme.primary, size: 18), const SizedBox(width: 6), Text(title, style: Theme.of(context).textTheme.labelLarge?.copyWith(fontWeight: FontWeight.w900)), const SizedBox(width: 6), Text('${items.length}', style: _muted())]), const SizedBox(height: 8), for (final item in items) _taskLine(item)]));
