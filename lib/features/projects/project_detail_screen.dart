...
class _ProjectDetailScreenState extends State<ProjectDetailScreen> {
              ...
              Padding(
                ...
                child: Row(
                  children: [
                    ...
                    IconButton(onPressed: _loading ? null : _refresh, icon: _loading ? const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2)) : const Icon(Icons.refresh_rounded)),
                  ],
                ),
              ),
              if (_error != null) Padding(padding: const EdgeInsets.fromLTRB(18, 0, 18, 8), child: _message(_error!, true)),
              Container(
                margin: const EdgeInsets.fromLTRB(18, 0, 18, 8),
                decoration: BoxDecoration(color: colors.surfaceVariant.withOpacity(.35), borderRadius: BorderRadius.circular(18)),
                child: const TabBar(
                  isScrollable: true,
                  dividerColor: Colors.transparent,
                  tabs: [Tab(text: 'Overview'), Tab(text: 'Tasks'), Tab(text: 'AI Tools'), Tab(text: 'Reports')],
                ),
              ),
              Expanded(
                child: TabBarView(
  ...
  Widget _projectCard() {
    ...
    return _card(Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        CircleAvatar(backgroundColor: colors.primary.withOpacity(.12), child: Icon(_project.isTeamProject ? Icons.groups_2_rounded : Icons.folder_rounded, color: colors.primary)),
        const SizedBox(width: 12),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(_project.title, style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w900)), Text('${_project.statusLabel} • ${_project.projectTypeLabel} • ${_project.deadlineLabel}', style: _muted())])),
      ]),
      if ((_project.description ?? '').trim().isNotEmpty) ...[const SizedBox(height: 12), Text(_project.description!.trim(), style: _muted())],
  ...
  Widget _aiHistoryCard() {
    return _section('AI Plan History', _aiPlans.isEmpty ? 'No AI plans yet' : '${_aiPlans.length} saved AI plan(s)', Icons.auto_awesome_rounded, _aiPlans.isEmpty ? Text('Generated AI plans for this project will appear here.', style: _muted()) : Column(children: [for (final plan in _aiPlans.take(4)) _tapLine(plan.summary, '${plan.generatedTaskCount} generated task(s) • ${_formatDateTime(plan.createdAt)}', Icons.psychology_alt_rounded, () => _showAiPlan(plan))]));
  }

  Widget _tasksCard() {
    return _section('Project Tasks', 'Open or delete project tasks', Icons.task_alt_rounded, _tasks.isEmpty ? Text('No tasks in this project yet.', style: _muted()) : Column(children: [for (final item in _tasks) _taskLine(item)]));
  }
  ...
  Widget _section(String title, String subtitle, IconData icon, Widget child) {
    final colors = Theme.of(context).colorScheme;
    return _card(Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(children: [CircleAvatar(backgroundColor: colors.primary.withOpacity(.10), child: Icon(icon, color: colors.primary)), const SizedBox(width: 12), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: Theme.of(context).textTheme.titleSmall?.copyWith(fontWeight: FontWeight.w900)), Text(subtitle, style: _muted())]))]), const SizedBox(height: 14), child]));
  }
