...
class _ProjectDetailScreenState extends State<ProjectDetailScreen> {
  ...
  Future<List<Map<String, dynamic>>> loadRiskHistory(int projectId) async {
    ...
    return asList(
      response,
    ).map(asMap).where((item) => item.isNotEmpty).toList();
  }
  ...
  Future<void> refresh() async {
    ...
    try {
      ...
      try {
        loadedProgress = await insightsApi.getProjectProgress(
      ...
      try {
        loadedActivities = await insightsApi.getProjectActivity(
  ...
  Future<void> applySmartSchedule() async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: const Text(
          'This will update task due dates using the previewed schedule.',
        ),
        actions: [
          TextButton(
  ...
  double get completionPercent {
    ...
    return (tasks.where((item) => item.task.isCompleted).length /
            tasks.length) *
        100;
  }
        ...
        child: RefreshIndicator(
          ...
          child: ListView(
            ...
            children: [
              ...
              if (error != null) ...[
                messageBox(context, error!, isError: true),
  ...
  Widget riskCard(BuildContext context) {
    ...
    final riskLevel =
        risk?.riskLevel ??
        textValue(
          latestSaved ?? const <String, dynamic>{},
          'risk_level',
          fallback: 'unknown',
        );
    ...
    return section(
      ...
      child: risk == null && latestSaved == null
          ? emptyText(
              context,
              'No saved risk analysis yet. Tap Analyze to save one and notify members if risk is high.',
            )
          : Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                if (latestSaved != null)
                  banner(
                    ...
                    'Latest saved • ${formatDateText(textValue(latestSaved, 'created_at'))}',
                    riskColor,
                  ),
                ...
                if (riskHistory.length > 1) ...[
                  ...
                  const SizedBox(height: 8),
                  for (final item in riskHistory.take(4))
                    historyRow(
                      context,
                      textValue(item, 'risk_level', fallback: 'medium'),
                      '${numValue(item, 'predicted_delay_days').round()} delay day(s) • ${formatDateText(textValue(item, 'created_at'))}',
                    ),
                ],
              ],
            ),
  ...
  Widget smartScheduleCard(BuildContext context) {
    ...
    return section(
      ...
      subtitle: preview == null
          ? 'Preview better task due dates'
          : '${preview.schedulableTaskCount} schedulable • ${preview.estimatedTotalHours.toStringAsFixed(1)}h',
      ...
      child: Column(
        ...
        children: [
            ...
            children: [
              ...
              Expanded(
                child: ElevatedButton.icon(
                  onPressed: applyingSchedule || preview == null
                      ? null
                      : applySmartSchedule,
          ...
          if (message != null)
            Padding(
              padding: const EdgeInsets.only(top: 10),
              child: Text(message!, style: smallMuted(context)),
            ),
          if (preview == null)
            Padding(
              padding: const EdgeInsets.only(top: 10),
          ...
          if (preview != null) ...[
            ...
            if (preview.warnings.isNotEmpty) ...[
              const SizedBox(height: 10),
              for (final warning in preview.warnings.take(2))
                Text(
                  'Warning: $warning',
  ...
  Widget aiPlanHistoryCard(BuildContext context) {
    return section(
      ...
      subtitle: aiPlans.isEmpty
          ? 'No AI plans yet'
          : '${aiPlans.length} saved AI plan(s)',
      ...
      child: aiPlans.isEmpty
          ...
          : Column(
              children: [
                for (final item in aiPlans.take(4)) aiPlanRow(context, item),
              ],
            ),
    );
  }
  ...
  Widget aiPlanRow(BuildContext context, AiPlanHistoryModel item) {
    return simpleInkRow(
      ...
      title: item.summary,
      subtitle:
          '${item.generatedTaskCount} generated task(s) • ${formatDateTime(item.createdAt)}',
  ...
  Widget activityCard(BuildContext context) {
    return section(
      ...
      subtitle: activities.isEmpty
          ? 'No recent activity'
          : '${activities.length} latest event(s)',
      icon: Icons.timeline_rounded,
      child: activities.isEmpty
          ? emptyText(
              context,
              'Project activity will appear here when tasks, comments, attachments, or reports change.',
            )
          : Column(
              children: [
                for (final item in activities) activityRow(context, item),
              ],
            ),
    );
  }
  ...
  Widget activityRow(BuildContext context, ProjectActivityModel item) {
    return simpleRow(
      ...
      title: item.message.trim().isEmpty
        ...
        children: [
          ...
          if (isReportReady) ...[
            fullButton(
              ...
              openingReport ? null : openLatestReport,
            ...
            fullOutlineButton(
              ...
              requestingReport ? null : requestReport,
            ...
            fullButton(
              ...
              requestingReport || isReportPending ? null : requestReport,
              isReportPending
                  ? Icons.hourglass_top_rounded
  ...
  Widget tasksCard(BuildContext context) {
    return section(
      ...
      child: tasks.isEmpty
          ? emptyText(context, 'No tasks in this project yet.')
          : Column(
              children: [
                for (final item in tasks.take(8)) taskRow(context, item),
              ],
            ),
    );
  }
  ...
  Widget membersCard(BuildContext context) {
    return section(
      ...
      child: members.isEmpty
          ? emptyText(context, 'No members found.')
          : Column(
              children: [
                for (final member in members) memberRow(context, member),
              ],
            ),
    );
  }
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(member.displayName, style: boldSmall(context)),
                Text(
  ...
  Widget scheduleTaskRow(
    BuildContext context,
    SmartScheduleTaskItemModel item,
  ) {
    return simpleRow(
      context,
      icon: item.isAfterProjectDeadline
          ? Icons.warning_amber_rounded
          : Icons.event_rounded,
      title: item.title,
      subtitle:
          '${item.priority} • ${item.estimatedHours.toStringAsFixed(1)}h • ${formatDateTime(item.suggestedDueDate)}',
      iconColor: item.isAfterProjectDeadline
  ...
  Widget historyRow(BuildContext context, String level, String subtitle) {
    return simpleRow(
      ...
      subtitle: subtitle,
      iconColor: colorForRisk(context, level),
    );
  }
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(subtitle, style: smallMuted(context)),
                  ],
                ),
              ),
              ?trailing,
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Text(
              text,
              style: smallMuted(context)?.copyWith(fontWeight: FontWeight.w800),
            ),
          ),
        ],
      ),
  ...
  Widget emptyText(BuildContext context, String value) =>
      Text(value, style: smallMuted(context));
  Widget messageBox(
    BuildContext context,
    String value, {
  ...
  TextStyle? smallMuted(BuildContext context) =>
      Theme.of(context).textTheme.bodySmall?.copyWith(
        color: Theme.of(context).colorScheme.onSurfaceVariant,
        fontWeight: FontWeight.w700,
        height: 1.35,
      );
  TextStyle? smallPrimary(BuildContext context) =>
      Theme.of(context).textTheme.bodySmall?.copyWith(
        color: Theme.of(context).colorScheme.primary,
        fontWeight: FontWeight.w900,
        height: 1.35,
      );
  TextStyle? boldSmall(BuildContext context) => Theme.of(
    context,
  ...
  IconData iconForActivity(String eventType) {
    ...
    if (value.contains('member') || value.contains('invite')) {
            ...
            card(
              ...
              Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 8),
                  Text('Generated ${formatDateTime(item.createdAt)}'),
                  if (item.inputPrompt.trim().isNotEmpty) ...[
                    const SizedBox(height: 10),
                    Text('Prompt: ${item.inputPrompt.trim()}'),
                  ],
                ],
              ),
            ),
              ...
              card(
                ...
                Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 10),
                    for (final rawTask in generatedTasks.take(20))
                      generatedPlanTaskRow(context, asMap(rawTask)),
                  ],
                ),
              ),
            ],
  ...
  Widget generatedPlanTaskRow(BuildContext context, Map<String, dynamic> task) {
    final title = textValue(
      task,
      'title',
      fallback: textValue(task, 'name', fallback: 'Untitled task'),
    );
    final description = textValue(
      task,
      'description',
      fallback: textValue(task, 'details'),
    );
    return Padding(
      padding: const EdgeInsets.only(bottom: 10),
      child: Row(
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(title, style: boldSmall(context)),
              ...
              Column(
                ...
                children: [
                  ...
                  if (tasksData.isEmpty)
                    emptyText(context, 'No tasks included.')
                  else
                    for (final rawTask in tasksData.take(20))
                      reportTaskRow(context, asMap(rawTask)),
                ],
              ),
            ),
          ],
  ...
  Widget reportTaskRow(BuildContext context, Map<String, dynamic> task) {
    return simpleRow(
      context,
      icon: Icons.task_alt_rounded,
      title: textValue(task, 'title', fallback: 'Untitled task'),
      subtitle:
          '${textValue(task, "status").replaceAll("_", " ")} • ${textValue(task, "priority")}',
    );
  }
